    ...
    <div className="max-w-md mx-auto mt-10 p-4 rounded-2xl shadow-xl bg-white text-zinc-900 font-[Gilroy_Bold] space-y-4">
      ...
      {step === "chooseType" && (
        <div className="space-y-4">
          <div className="space-y-1">
            <Button className="w-full" onClick={() => { setPaymentType("fiat"); setStep("chooseCurrency"); }}>Pay by card or bank transfer</Button>
      ...
      {step === "chooseCurrency" && (
        <div className="space-y-2">
          {currency && (
            <div className="border p-3 rounded-xl text-sm bg-zinc-50">
              <div className="flex justify-center mt-2">
                <QRCode value={(availableMethods.find(m => m.currencies.includes(currency)) || {}).address || ''} size={100} />
              </div>
              <p className="text-zinc-700">Estimated total:</p>
              <p className="font-bold">{amount} {currency}</p>
              <p className="mt-2 text-zinc-700">Recipient address (preview):</p>
              <div className="flex items-center justify-between gap-2">
                <span className="truncate max-w-[200px] text-xs text-zinc-500">{(availableMethods.find(m => m.currencies.includes(currency)) || {}).address}</span>
                <Copy className="w-4 h-4 cursor-pointer text-zinc-400" onClick={() => copyToClipboard((availableMethods.find(m => m.currencies.includes(currency)) || {}).address || '')} />
              </div>
            </div>
          )}
          ...
          <Select onValueChange={(val) => { setCurrency(val); setStep("chooseMethod"); }}>
            ...
            <SelectContent>
              {(paymentType === "crypto" ? cryptoCurrencies : fiatCurrencies).map(code => (
